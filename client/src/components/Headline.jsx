...
const Headline = () => {
  ...
  const pageSize = 6; // Number of news articles per page
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const categoryParam=param.category ? `&category=${param.category}` : "";
        console.log(categoryParam)
        const response = await fetch(`http://localhost:3000/top-headlines?&language=en${categoryParam}&page=${page}&pageSize=${pageSize}`);
        ...
        if (myJson.data.articles) {
          console.log(myJson.data.articles)
